...
def _z_value(confidence_level: float) -> float:
    ...
    # Fall back to 95% if someone passes a weird value
    ...
    *,
    ...
    while remaining > 0:
        ...
        if antithetic:
            ...
            z_full = np.concatenate([z_half, -z_half])[:n_batch]
            z = z_full
        else:
        ...
        # Control variate (discounted terminal stock); E[disc*S_T] = S0
        y = disc * st
    ...
    if used_cv:
        # Optimal coefficient b = Cov(X,Y) / Var(Y)
        b = cov_xy / var_y
        # Control variate adjustment: X_adj = X - b (Y - E[Y])
        ey = s0  # E[exp(-rT) S_T] under risk-neutral GBM
        price = mean_x - b * (mean_y - ey)

        # Var(X - b(Y - E[Y])) = Var(X) + b^2 Var(Y) - 2 b Cov(X,Y)
        var_adj = var_x + (b * b) * var_y - 2.0 * b * cov_xy
        var_adj = max(var_adj, 0.0)  # numeric safety
